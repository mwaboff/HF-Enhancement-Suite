...
function citePage(){
    if(document.URL.indexOf("hid=") != -1){
        return '[url='+document.URL.replace('nsfw.','www.')+'][b]'+$('.navigation .active').text()+' Help Document[/b][/url]';
    }else{
...
function getForumNewPosts(fid){
    ...
    debugPrint("[Favorites] Attempting to load forum data. fid="+fid);
    if(document.URL.indexOf("nsfw.") != -1){
    ...
    try{
        ...
        debugPrint('[Favorites] Error attempting to get info from fid='+fid+': '+err.message);
        return;
    }
}
...
function addNewThreads(newdata){
    ...
    newHTML += '\n<!-- %INSERT POSTS% -->';
    //debugPrint(document.getElementById("insertposts").innerHTML);
...
function addRemoveFavForum(){ // HOLDERTURTLE
    ...
    if(fname.indexOf('Board Message') != -1){
        ...
        debugPrint('[HFES] Forum unfavorited!');
        $('#favforum small').text('Favorite this forum');
    }else{
        favFids[fid] = fname;
        GM_SuperValue.set('favList', favFids);
        debugPrint('[HFES] Forum favorited!');
...
function threadPageMods(){
    ...
    if(GM_config.get("ratingInfo")){
        debugPrint("Loading Detailed Raiting Mod...");
